...
bool IsHexToDec(std::string_view hexValue, char& result){
    std::stringstream ss;
    ss << std::hex << hexValue;
    ss >> result;
    return !ss.fail();
...
std::string DecodeURI(std::string_view sv){
    std::string res;
    res.reserve(sv.size());
    char prc = '%';
    char ch;
    for (auto pos = sv.find(prc, 0); pos != std::string::npos; pos = sv.find(prc, 0)) {
        if (pos + 3 > sv.size() || !IsHexToDec(sv.substr(pos + 1, 2), ch)) {
            return {};
        }
...
Response MakeFileResponse(std::string query, const fs::path& path, const StringRequest& req){
    using namespace http;
    using namespace std::literals;
    // Проверка пути в запросе
    fs::path rel_path = {DecodeURI(query)};
    fs::path abs_path = fs::weakly_canonical(path / rel_path);
    if (!IsSubPath(abs_path, path)) {
