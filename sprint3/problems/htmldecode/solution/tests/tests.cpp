...
TEST_CASE("Simple text wo mnemonics", "[HtmlDecode]") {
    const std::string orig{ "String" };
    const std::string expected{ "String" };
    CHECK(HtmlDecode(orig) == expected);
}

TEST_CASE("Simple text w mnemonics", "[HtmlDecode]") {
...
TEST_CASE("Capital letters mnemonics", "[HtmlDecode]") {
